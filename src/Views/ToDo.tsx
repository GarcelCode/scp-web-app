...
export function ToDo() {
  ...
  function todosFilter(obj:ITodo){
    if(obj.text){
      return true
    }
  }
    ...
    <div className="flex flex-col justify-center">
        ...
        <section className="w-full flex justify-center">
            <form className="flex items-center gap-2">
                <input onChange={changeHandler} placeholder="Escribe una tarea" className="text-slate-200 text-lg p-2 rounded-lg border-none bg-slate-600 w-4/5 focus:border-none focus:border-slate-700 focus:text-slate-100" value={todo.text}/>
        ...
        <ul className="mt-4 mb-5 flex flex-col gap-2 items-center">
          {todos.map((item:ITodo, index:any)=>(
            <li key={index}>
              <TodoCard text={item.text} done={item.done} action={removeHandler}/>
            </li>
          ))}
        </ul>
    </div>
